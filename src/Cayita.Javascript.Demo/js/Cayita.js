(function() {
	...
	var $Cayita_UI_ElementBase$1 = function(T) {
		...
		$type.prototype = {
			...
			popover: function(options) {
				new $Cayita_UI_Popover.$ctor2(this.getMainElement(), options);
				return this;
			},
			popover$1: function(config) {
				var options = $Cayita_UI_PopoverOptions.$ctor();
				config(options);
	...
	var $Cayita_UI_GroupBase$2 = function(TField, TInput) {
		var $type = function(type, parent, text, fieldName, items, inline) {
			...
			this.$il = false;
			this.$tp = null;
			this.$3$ChangedField = function(f, i, ch, evt) {
			};
		...
		$type.prototype = {
			...
			$init: function(parent) {
				...
				this.get_controls().jQuery().on('change', '[type=' + this.$tp + ']', ss.mkdel(this, function(evt) {
					var i = evt.target;
					var $t1 = $Cayita_UI_GroupItem.$ctor();
					$t1.value = i.value;
					$t1.text = $Extensions.text(i.parentNode);
					var item = $t1;
					var checkd = $SystemExtensions.get$1(Boolean).call(null, i, 'checked');
					this.onChanged(item, checkd, evt);
				}));
			},
			add_changed: function(value) {
				this.$3$ChangedField = ss.delegateCombine(this.$3$ChangedField, value);
			},
			remove_changed: function(value) {
				this.$3$ChangedField = ss.delegateRemove(this.$3$ChangedField, value);
			},
			onChanged: function(item, checkd, evt) {
				this.$3$ChangedField(this, item, checkd, evt);
			},
			...
			$addItem: function(item) {
				var input = ss.createInstance(TInput).value$1(item.value);
				(new $Cayita_UI_Label.$ctor1(this.get_controls().element$1())).className$1(this.$tp + (this.$il ? ' inline' : '')).text$1(item.text).for$2(input.get_id()).append$1(input);
				if (!ss.isNullOrEmptyString(this.$nm)) {
					input.name$1(this.$nm);
				}
			}
		};
	...
	var $Cayita_UI_InputBase$1 = function(T) {
		var $type = function() {
			this.$3$ChangedField = function(f, evt) {
			};
			ss.makeGenericType($Cayita_UI_ElementBase$1, [T]).call(this);
		...
		$type.prototype = {
			createInput: function(parent, type, tagname) {
				...
				$(this.element$1()).on('change', ss.mkdel(this, function(evt) {
					this.onChanged(evt);
				}));
			},
			add_changed: function(value) {
	...
	var $Cayita_UI_Panel = function() {
		...
		this.$dobject = null;
		this.$robject = null;
		this.$3$HeaderField = null;
		this.$3$BodyField = null;
	...
	$Cayita_UI_Panel.prototype = {
		get_header: function() {
			return this.$3$HeaderField;
		},
		set_header: function(value) {
			this.$3$HeaderField = value;
		},
		get_body: function() {
			return this.$3$BodyField;
		},
		set_body: function(value) {
			this.$3$BodyField = value;
		},
		$init: function(config) {
			this.createElement('div', null);
			var el = this.element$1();
			el.className = 'c-panel';
			this.set_header((new $Cayita_UI_Div.$ctor1(el)).className$1('c-panel-header'));
			this.set_body((new $Cayita_UI_Div.$ctor1(el)).className$1('c-panel-content'));
			var self = this;
			this.$pc = config;
			...
			var hd = this.get_header().element$1();
			this.$closeIcon = new $Cayita_UI_Icon.$ctor3(hd, ss.mkdel(this, function(icon) {
			...
			this.$collapseIcon = new $Cayita_UI_Icon.$ctor3(hd, ss.mkdel(this, function(icon1) {
				icon1.className = this.$pc.collapseIconClass;
				$(icon1).on('click', ss.mkdel(this, function(evt1) {
					evt1.preventDefault();
					this.$pc.collapseIconHandler(self, !this.get_body().isVisible());
				}));
			...
			$(hd).append(this.$captionElement);
			this.jQuery().css('left', this.$pc.left).css('top', this.$pc.top);
			this.get_body().jQuery().css('width', this.$pc.width).css('height', this.$pc.height);
			this.jQuery().css('z-index', '0');
			this.$initDraggable();
			...
			this.jQuery().click(ss.mkdel(this, function(evt2) {
				var zI = this.jQuery().css('z-index');
				var hZ = parseInt(zI);
				var hE = this.element$1();
				...
				$(hE).css('z-index', zI);
				this.jQuery().css('z-index', ((hZ > 0) ? hZ.toString() : '1'));
			}));
			if (this.$pc.overlay) {
				this.jQuery().css('position', 'fixed');
			}
		},
		...
		overlay: function(value) {
			...
			if (value) {
				...
				this.jQuery().css('position', 'relative');
			}
			return this;
		},
		...
		left: function(value) {
			this.$pc.top = value;
			this.jQuery().css('left', value);
			return this;
		},
		top: function(value) {
			this.$pc.top = value;
			this.jQuery().css('top', value);
			return this;
		},
		...
		height: function(value) {
			...
			this.get_body().jQuery().css('height', value);
			return this;
		},
		collapse: function() {
			var collapsed = !this.get_body().isVisible();
			this.onCollapsed(collapsed);
			this.get_body().jQuery().toggle();
		...
		append$7: function(content) {
			this.get_body().append$3(content);
			return this;
		},
		append$6: function(content) {
			this.get_body().append$2(content);
			return this;
		},
		append$5: function(T) {
			return function(content) {
				this.get_body().append$2(content.element());
				return this;
			};
		},
		element$1: function() {
		...
		$initDraggable: function() {
			this.$dobject = this.jQuery().draggable();
			...
			this.$dobject.draggable('option', 'handle', this.get_header().element$1());
		},
		$initResizable: function() {
			this.$robject = this.get_body().jQuery().resizable();
			this.$robject.resizable('option', 'alsoResize', this.jQuery());
		}
	};
	...
	////////////////////////////////////////////////////////////////////////////////
	// Cayita.UI.Popover
	var $Cayita_UI_Popover = function(parent, options) {
		this.$p = null;
		this.$init(parent.getMainElement(), options);
	};
	$Cayita_UI_Popover.prototype = {
		show: function() {
			this.$p['popover']('show');
		},
		hide: function() {
		...
		toggle: function() {
			this.$p['popover']('toggle');
		},
		destroy: function() {
			this.$p['popover']('destroy');
		},
		$init: function(parent, options) {
			this.$p = $(parent);
			this.$p['popover'](options);
		}
	};
	$Cayita_UI_Popover.$ctor2 = function(parent, options) {
		this.$p = null;
		this.$init(parent, options);
	};
	$Cayita_UI_Popover.$ctor1 = function(parent, options) {
		$Cayita_UI_Popover.$ctor3.call(this, parent.getMainElement(), options);
	};
	$Cayita_UI_Popover.$ctor3 = function(parent, options) {
		this.$p = null;
		var opt = $Cayita_UI_PopoverOptions.$ctor();
		options(opt);
		this.$init(parent, opt);
	};
	$Cayita_UI_Popover.$ctor2.prototype = $Cayita_UI_Popover.$ctor1.prototype = $Cayita_UI_Popover.$ctor3.prototype = $Cayita_UI_Popover.prototype;
	////////////////////////////////////////////////////////////////////////////////
	// Cayita.UI.PopoverOptions
	var $Cayita_UI_PopoverOptions = function() {
	};
	$Cayita_UI_PopoverOptions.createInstance = function() {
		return $Cayita_UI_PopoverOptions.$ctor();
	};
	$Cayita_UI_PopoverOptions.$ctor = function() {
		var $this = {};
	...
	ss.registerClass(global, 'Cayita.UI.Popover', $Cayita_UI_Popover);
	ss.registerClass(global, 'Cayita.UI.PopoverOptions', $Cayita_UI_PopoverOptions);
